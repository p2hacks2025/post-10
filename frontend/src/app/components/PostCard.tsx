...
const getDynamicStyle = (point: number) => {
  // 1. 神投稿 (point 10〜)
  if (point >= 1.0) return "text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 via-yellow-500 to-orange-500 drop-shadow-[0_0_15px_rgba(251,191,36,0.8)] animate-bounce leading-tight py-2";

  // 2. 優良投稿 (point 5〜)
  if (point >= 0.5) return "text-3xl font-extrabold text-blue-300 drop-shadow-md leading-snug";

  // 3. 軽度の低評価 (-1 〜 -4)
  if (point <= -0.5 && point > -1.0) {
